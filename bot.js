...
// --- RENDER 7/24 AYARI (KAPANMAMASI İÇİN) ---
const express = require('express'); 
const app = express();
const port = process.env.PORT || 10000; 

app.get('/', (req, res) => res.send('xbabapirobot Aternos üzerinde tam kapasite aktif!'));
app.listen(port, '0.0.0.0', () => {
    console.log(`Web sunucusu ${port} portunda baslatildi.`);
});

// --- AYARLAR ---
const SUNUCU_IP = 'cagripro35.aternos.me'; // Aternos IP'n
    ...
    bot.on('chat', async (username, message) => {
        ...
        switch (command) {
            ...
            case 'yardım':
                ...
                setTimeout(() => bot.chat("🛠 4/4: %aktif, %version, %pingim, %kimim, %neredeyim, %saat, %tps"), 4500);
                break;

            case 'v-yardım':
                if (!isV) return;
                bot.chat(`/msg ${username} 💎 V 1/3: %v, %v-k, %v-list, %mevlana, %dur, %tp, %gel`);
                setTimeout(() => bot.chat(`/msg ${username} 💎 V 2/3: %izle, %koru, %dans, %spam, %afk, %can-bak`), 1500);
                setTimeout(() => bot.chat(`/msg ${username} 💎 V 3/3: %god-mod, %getir, %envanter`), 3000);
                break;
            ...
            case 'espri': bot.chat("Temel bir gün ne demiş? 'Ben gidiyorum' demiş."); break;
            case 'discord': bot.chat("📢 Discord adresimiz yakında!"); break;
        }
    });

    bot.once('spawn', () => {
        console.log('Bot Aternos sunucusuna giriş yaptı!');
